...
function pickTopic(emailLogs, subscriber, topics) {
...
async function sendForSubscriber(db, subscriber) {
  ...
  const topic = pickTopic(db.email_logs, subscriber, topics);
  const script = await generateConversation({ topic, level: subscriber.level });
